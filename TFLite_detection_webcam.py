...
while True:
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            #initiate middle area
            minPos = 3*imW/8
            maxPos = 5*imW/8
            
            #Check whether detected anything as cow
            if object_name == 'cow':
                if xmin > 0 & xmin < minPos:
                    config.pigCase = 0      #too left
                elif xmin > minPos & xmax < maxPos:
                    config.pigCase = 1      #middle
                elif xmax > maxPos & xmax < imW:
                    config.pigCase = 2      #too right
                else: 
                    config.pigCase = -1     #no where
